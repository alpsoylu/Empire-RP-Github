...
NDiplomacy = {
	...
	TRUCE_BREAK_COST_PP = 1000,						-- Base cost in PP of breaking a truce by joining a war or accepting a call to war ( you cannot declare war yourself until the truce if up ), this is then multiplied by the time left on the truce ( so once half the truce is up it only cost 50% of this )
...
NResistance = {
	...
	SUPPRESSION_NEEDED_BY_RESISTANCE_POINT = 0.37, -- Number of suppression point we need for each 1% of resistance
	...
	SUPPRESSION_NEEDED_UPPER_CAP = 25.0, -- if resistance is greater than this value then we always act as though it is at the define for the purpose of suppresion requirements
	
	GARRISON_MANPOWER_LOST_BY_ATTACK = 0.0075, 	-- Ratio of manpower lost by garrison at each attack on garrison (this number will be reduced by the hardness of garrison template)
...
NProduction = {
	...
	BASE_FACTORY_SPEED = 5.5, 				-- Base factory speed multiplier (how much hoi3 style IC each factory gives).
...
NNavy = {
	...
	NAVAL_COMBAT_AIR_LOW_AA_TARGET_SCORE = 5, 					-- A global factor that applies after all others, right before the sub detection contributed by plane is added to the global sub detection of a combatant
...
NIntel = {
	...
	NAVAL_SUPREMACY_INTEL_LOW_SUPREMACY_MIN_PENALTY = 0.5,								-- we need more intel than this to get any supremacy
